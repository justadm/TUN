...
func main() {
	...
	healthAddr := flag.String("health-addr", "", "optional health listen address, for example 127.0.0.1:18081")
	...
	log.Printf("runtime server listening on %s tun=%q", *addr, *tunName)
	err = runtime.RunServer(
		...
		runtime.ClientOptions{
			...
			// Explicit server-side direction mapping.
	...
	if err != nil && err != context.Canceled && !errors.Is(err, net.ErrClosed) {
		log.Fatalf("runtime server exited with error: %v", err)
	}
	if *supportBundleOut != "" {
	...
	log.Printf("runtime server stopped")
}
