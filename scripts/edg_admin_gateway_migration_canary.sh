...
if not peer_id:
    print(json.dumps({"ok": False, "error": "no_active_peer_for_admin_migration_canary"}, ensure_ascii=False))
    raise SystemExit(1)

resp = post_form(base + f"/admin/action/?token={urllib.parse.quote(admin_token)}", {
    "id": peer_id,
    "action": "migrate_gateway",
    "gateway": gateway,
    "uplink": uplink,
    "next": f"/admin/peers/{peer_id}/",
})
html = resp["html"]
match = re.search(r"<code id='peerId'>([^<]+)</code>", html)
new_peer_id = match.group(1).strip() if match else ""
report = {"peer_id": peer_id, "gateway": gateway, "uplink": uplink, "status": resp["status"], "new_peer_id": new_peer_id}
if not new_peer_id:
    print(json.dumps({"ok": False, "stage": "admin_migrate", "report": report}, ensure_ascii=False))
    raise SystemExit(1)

time.sleep(2)
routing = api("GET", shadow + f"/peers/{new_peer_id}/routing", shadow_token)
report["routing"] = routing
removed = api("POST", local + f"/peers/{new_peer_id}/remove", api_token, {})
